 ...
 */
export async function assertSchemaCompatibility(client, schema = DEFAULT_SCHEMA, options = {}) {
    var _a, _b, _c, _d;
    ...
    try {
        ({ rows } = await client.query(`SELECT COALESCE(MAX(version), 0)::int AS version,
         ...
         FROM ${quotedSchema}.migration`, [BULLMQ_MAJOR_VERSION]));
    }
    catch (err) {
        if (err.code === '42P01') {
    ...
    const minimumClientVersion = (_c = state === null || state === void 0 ? void 0 : state.min_client_version) !== null && _c !== void 0 ? _c : BULLMQ_MAJOR_VERSION;
    if (minimumClientVersion > BULLMQ_MAJOR_VERSION) {
        throw new SchemaVersionMismatchError(minimumClientVersion, BULLMQ_MAJOR_VERSION);
    }
...
async function ensureLedgerTable(client) {
    ...
    await client.query('UPDATE migration SET min_client_version = $1 WHERE min_client_version IS NULL', [BULLMQ_MAJOR_VERSION]);
...
async function getMinimumClientVersion(client) {
    ...
    const { rows } = await client.query(`SELECT COALESCE(MAX(min_client_version), $1)::int AS min_client_version
       FROM migration`, [BULLMQ_MAJOR_VERSION]);
    return (_b = (_a = rows[0]) === null || _a === void 0 ? void 0 : _a.min_client_version) !== null && _b !== void 0 ? _b : BULLMQ_MAJOR_VERSION;
}
